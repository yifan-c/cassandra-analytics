...
public enum CassandraVersion
{
    ...
    // The oldest Cassandra version (by versionNumber) whose SSTables this version can read. This is a fact
    // defined per release in the Cassandra codebase (the SSTable format's earliest-supported version), and is
    // replicated here rather than inferred. e.g. C* 6.0 (when added) would set this to 40, so it reads 4.0/4.1/5.0/6.0.
    ...
    /**
     * Get the set of SSTable version strings that this Cassandra version can read: its own native versions plus
     * those of every Cassandra version down to its configured {@link #lowestCompatibleVersionNumber}. For example
     * C* 5.0 (lowest-compatible 4.0) reads 4.0/4.1/5.0 SSTables but not 3.0.
     *
